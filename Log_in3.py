...
db = database.connect('127.0.0.1', 'root', 'KurwaScript', 'bazadate')
...
def OpenReg():
	...
	def reg(*args):
		...
		e_name = i_name.get()
		e_price = i_price.get()
		Lowroot = database.connect('127.0.0.1', 'root', 'KurwaScript', 'bazadate')
...
def In():
	...
	db = database.connect('127.0.0.1', 'root', 'KurwaScript', 'bazadate')
	cursor = db.cursor()
	#t = cursor.execute("SELECT Login FROM users WHERE Login = '{e_code}'")
	print(cursor.execute("SELECT Password FROM users WHERE UserId = 3"))'''

	cursor = db.cursor()
	cursor.execute('SELECT Login FROM users WHERE UserId = 2;')
